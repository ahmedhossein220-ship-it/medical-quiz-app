...
def run_quiz():
    ...
    print("="*120)
    print("      PRACTICAL ULTIMATE TRAINER - JSON VERSION")
    print("="*120)
    print(f"{len(all_questions)} Fragen geladen Ya Hobi. Viel Erfolg!")
    ...
    while current_round:
        ...
        for i, item in enumerate(current_round):
            ...
            if ans.upper() == item["a"].upper():
                ...
                streak = 0
                print(f"❌ FALSCH! Die Lösung ist: {item['a']}")
        ...
        if not wrong_pool:
            ...
            print(f"\nEnde Runde {round_no}.Baka {len(wrong_pool)} Fehler werden wiederholt.")
            current_round = wrong_pool
            round_no += 1
            input("Niggi Drücke Enter für die nächste Runde...")
